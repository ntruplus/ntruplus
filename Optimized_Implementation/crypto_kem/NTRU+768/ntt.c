...
**************************************************/
void basemul(int16_t r[8], const int16_t a[8], const int16_t b[8], uint32_t zeta)
{
    uint32_t z1 = zeta;
    uint32_t z2 = -zeta;

    uint32_t A0=a[0]*NTRUPLUS_QINV, A1=a[1]*NTRUPLUS_QINV, A2=a[2]*NTRUPLUS_QINV, A3=a[3]*NTRUPLUS_QINV;
    uint32_t B0=a[4]*NTRUPLUS_QINV, B1=a[5]*NTRUPLUS_QINV, B2=a[6]*NTRUPLUS_QINV, B3=a[7]*NTRUPLUS_QINV;
    ...
    r2 = plantard_reduce_acc((int32_t)r2*z1 + A0*b[2] + A1*b[1] + A2*b[0]);

    r4 = plantard_reduce_acc((int32_t)r4*z2 + B0*b[4]);
    r5 = plantard_reduce_acc((int32_t)r5*z2 + B0*b[5] + B1*b[4]);
    r6 = plantard_reduce_acc((int32_t)r6*z2 + B0*b[6] + B1*b[5] + B2*b[4]);
...
*                a[0..3], b[0..3], c[0..3]  in Z_q[X]/(X^4 - zeta),
*                a[4..7], b[4..7], c[4..7]  in Z_q[X]/(X^4 + zeta).
*              The output r uses the same layout:
*                r[0..3] = a[0..3]*b[0..3] + c[0..3] mod (X^4 - zeta),
*                r[4..7] = a[4..7]*b[4..7] + c[4..7] mod (X^4 + zeta).
*
* Arguments:   - int16_t r[8]:       output (two polynomials)
*              - const int16_t a[8]: first input (two polynomials)
*              - const int16_t b[8]: second input (two polynomials)
*              - const int16_t c[8]: addend (two polynomials)
*              - uint32_t zeta:      defines X^4 ± zeta
**************************************************/
void basemul_add(int16_t r[8], const int16_t a[8], const int16_t b[8], const int16_t c[8], uint32_t zeta)
{
    uint32_t z1 = zeta;
    uint32_t z2 = -zeta;
    ...
    r6 = plantard_reduce_acc((uint32_t)c[6]*NTRUPLUS_R + (uint32_t)r6*NTRUPLUS_Rsq);
    r7 = plantard_reduce_acc((uint32_t)c[7]*NTRUPLUS_R + (uint32_t)r7*NTRUPLUS_Rsq);
