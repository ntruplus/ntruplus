...
static inline int poly_fqinv_batch(int16x8_t r[36], int16x8_t con)
{
    int16x8_t c[36];
    int16x8_t inv;
    ...
    if (!vminvq_u16(vreinterpretq_u16_s16(c[35]))) return 1;

    inv = fqinv_neon(c[35], con);
...
static void poly_baseinv_2(poly *r, int16x8_t *den, int16x8_t con)
{
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        r0 = fqmul_neon(r0, den[i], con);
        r1 = fqmul_neon(r1, den[i], con);
        r2 = fqmul_neon(r2, den[i], con);
