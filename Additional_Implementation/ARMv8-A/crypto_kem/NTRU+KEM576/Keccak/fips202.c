 ...
 */
 ...
 static void keccak_init(uint64_t *s) {
     KeccakP1600_Initialize((KeccakP1600_align512plain64_state*) s);
     s[25] = 0;
 }
 ...
 static void keccak_absorb(uint64_t *s, uint32_t r, const uint8_t *m,
     ...
     if (s[25] && mlen + s[25] >= r) {
         c = r - s[25];
         KeccakP1600_AddBytes((KeccakP1600_align512plain64_state*)s, m, (unsigned int)s[25], (unsigned int)c);
         KeccakP1600_Permute_24rounds((KeccakP1600_align512plain64_state*)s);
         mlen -= c;
         m += c;
         s[25] = 0;
     }
 ...
 #else
     while (mlen >= r) {
         KeccakP1600_AddBytes((KeccakP1600_align512plain64_state*)s, m, 0, r);
         KeccakP1600_Permute_24rounds((KeccakP1600_align512plain64_state*)s);
         mlen -= r;
         m += r;
     }
     ...
     KeccakP1600_AddBytes((KeccakP1600_align512plain64_state*)s, m, (unsigned int)s[25], (unsigned int)mlen);
     s[25] += mlen;
 }
 ...
 static void keccak_finalize(uint64_t *s, uint32_t r, uint8_t p) {
     ...
     KeccakP1600_AddByte((KeccakP1600_align512plain64_state*)s, p, (unsigned int)s[25]);
     KeccakP1600_AddByte((KeccakP1600_align512plain64_state*)s, 0x80, (unsigned int)(r - 1));
     s[25] = 0;
 }
 ...
 static void keccak_squeeze(uint8_t *h, size_t outlen, uint64_t *s, uint32_t r) {
     while (outlen > s[25]) {
         KeccakP1600_ExtractBytes((KeccakP1600_align512plain64_state*)s, h, (unsigned int)(r - s[25]),
                                       (unsigned int)s[25]);
         KeccakP1600_Permute_24rounds((KeccakP1600_align512plain64_state*)s);
         h += s[25];
         outlen -= s[25];
 ...
 /* shake128 */
 void shake128_init(keccak_state *state) { keccak_init((uint64_t*)state); }
 
 void shake128_absorb(keccak_state *state, const uint8_t *input, size_t inlen) {
     keccak_absorb((uint64_t*)state, SHAKE128_RATE, input, inlen);
 }
 
 void shake128_finalize(keccak_state *state) {
     keccak_finalize((uint64_t*)state, SHAKE128_RATE, 0x1F);
 }
 
 void shake128_squeeze(uint8_t *output, size_t outlen, keccak_state *state) {
     keccak_squeeze(output, outlen, (uint64_t*)state, SHAKE128_RATE);
 }
 ...
 /* shake256 */
 void shake256_init(keccak_state *state) { keccak_init((uint64_t*)state); }
 
 void shake256_absorb(keccak_state *state, const uint8_t *input, size_t inlen) {
     keccak_absorb((uint64_t*)state, SHAKE256_RATE, input, inlen);
 }
 
 void shake256_finalize(keccak_state *state) {
     keccak_finalize((uint64_t*)state, SHAKE256_RATE, 0x1F);
 }
 
 void shake256_squeeze(uint8_t *output, size_t outlen, keccak_state *state) {
     keccak_squeeze(output, outlen, (uint64_t*)state, SHAKE256_RATE);
 }
 ...
 /* sha3-256 */
 void sha3_256_init(keccak_state *state) { keccak_init((uint64_t*)state); }
 
 void sha3_256_absorb(keccak_state *state, const uint8_t *input, size_t inlen) {
     keccak_absorb((uint64_t*)state, SHA3_256_RATE, input, inlen);
 }
 
 void sha3_256_finalize(uint8_t *output, keccak_state *state) {
     keccak_finalize((uint64_t*)state, SHA3_256_RATE, 0x06);
     keccak_squeeze(output, 32, (uint64_t*)state, SHA3_256_RATE);
 }
