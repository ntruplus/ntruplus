...
void ntt(int16_t a[NTRUPLUS_N])
{
	...
	{
		t = fqmul(zeta, a[i + 432]);

		a[i + 432] = (a[i] + a[i + 432] - t);
		a[i      ] = (a[i] + t);
	}
...
void invntt(int16_t a[1024])
{
	...
	for(int i = 0; i < 432; i++)
	{
		...
		a[i      ] = fqred16(fqmul((1 << 11)*3073  % 3457, t1 - t2));
		a[i + 432] = fqred16(fqmul((1 << 12)*3073 % 3457, t2));

		a[i      ] = fqred16(fqmul(1764, t1 - t2));
		a[i + 432] = fqred16(fqmul(71, t2));	
