...
void ntt(int16_t a[768])
{
	...
	for(int i = 0; i < 384; i++)
	{
		t = fqmul(zeta, a[i + 384]);

		a[i + 384] = (a[i] + a[i + 384] - t);
		a[i      ] = (a[i] + t);
	}

	for(int start = 0; start < 768; start += 384)
	{
		zeta1 = zetas[k++]; //alpha
