...
void poly_basemul(poly *r, const poly *a, const poly *b)
{
	...
	{
		basemul(r->coeffs + 6*i, a->coeffs + 6*i, b->coeffs + 6*i, zetas[192 + i]);
		basemul(r->coeffs + 6*i + 3, a->coeffs + 6*i + 3, b->coeffs + 6*i + 3, -zetas[192 + i]);
	}
}
...
int poly_baseinv(poly *r, const poly *a)
{
	...
	for(int i = 0; i < NTRUPLUS_N/6; ++i)
	{
		result += baseinv(r->coeffs + 6*i, a->coeffs + 6*i, zetas[192 + i]);
		result += baseinv(r->coeffs + 6*i + 3, a->coeffs + 6*i + 3, -zetas[192 + i]);
	 }
