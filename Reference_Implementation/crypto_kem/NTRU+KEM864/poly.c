...
void poly_basemul(poly *c, const poly *a, const poly *b)
{
	...
	{
		basemul(c->coeffs + 6*i, a->coeffs + 6*i, b->coeffs + 6*i, zetas[144 + i]);
		basemul(c->coeffs + 6*i + 3, a->coeffs + 6*i + 3, b->coeffs + 6*i + 3, -zetas[144 + i]);
	}
}
