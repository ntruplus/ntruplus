...
void poly_basemul(poly *c, const poly *a, const poly *b)
{
	...
	{
		basemul(c->coeffs + 6*i, a->coeffs + 6*i, b->coeffs + 6*i, zetas[95 + i]);
		basemul(c->coeffs + 6*i + 3, a->coeffs + 6*i + 3, b->coeffs + 6*i + 3, -zetas[95 + i]);
	}
}
...
int poly_baseinv(poly *b, const poly *a)
{
	...
	for(i = 0; i < NTRUPLUS_N/6; ++i)
	{
		r += baseinv(b->coeffs + 6*i, a->coeffs + 6*i, zetas[95 + i]);
		r += baseinv(b->coeffs + 6*i + 3, a->coeffs + 6*i + 3, -zetas[95 + i]);
	 }
